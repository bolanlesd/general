...
tgperm() {
    ...
    if [ "$1" == "destroy" ]; then
        ...
        echo "Extracting AWS Permissions for Destroy..."
        grep -o "rpc.method=[^ ]* \(rpc.service=[^ ]*\)\?" log_destroy.log | sort | uniq > all_destroy_permissions.txt
        ...
        }' all_destroy_permissions.txt > extracted_destroy_permissions.txt

        echo "Destroy permissions extracted to extracted_destroy_permissions.txt"

        rm -f all_destroy_permissions.txt
